...
func main() {
	...
	* 参照型としてのスライス
	...
	a6 := []int{1, 2, 3}
	pow2(a6)
	fmt.Println(a6) // [1 4 9]

	/*
	* スライスの注意点
	 */
	// 配列やスライスを元に新しいスライスを生成する場合は挙動に注意が必要
	// 生成したスライスが参照するデータは元の配列そのものでコピーはされない
	// その為、元の配列の要素を変更すると新たに生成したスライスの値も変更される

	// しかし、新しいスライスにappendした場合にはそのタイミングで参照先の配列が別のメモリ上の配列に差し変わる
	// スライスの自動拡張による挙動の問題は以下の様にまとめることができる

	// ↓の s1 と s0 が同じ配列データを共有するかは不定
	// s1 := append(s0, x)
}
...
* 参照型としてのスライス
 */
// 配列の場合は引数を受け取っても「値渡し」によってコピーされ、メモリ領域の異なる別物
// よって受け取った配列の値を操作することは不可能
func pow(a [3]int) {
	// 配列の各要素を2乗する
	for i, v := range a {
		a[i] = v * v
...
// スライスの場合は「参照渡し」によって同一メモリ内の領域に対する処理となる
// よって受け取ったスライスの値を直接操作することが可能
func pow2(a []int) {
	// 配列の各要素を2乗する
	for i, v := range a {
		a[i] = v * v
